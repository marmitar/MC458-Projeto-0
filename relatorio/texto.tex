\section{Definições}
    ...
    Um móbile $M$, com objetos $E$ e $D$ presos na barra, está em equilíbrio se $P_e \cdot D_e = P_d \cdot D_d$ e todos os seus submóbiles também estão em equilíbrio, sendo que cada objeto $E$ ou $D$ pode ser um submóbile ou um peso simples. No entanto, essa definição só faz sentido para móbiles, mas ela pode ser expandida para pesos simples, considerando esse tipo de objeto como sempre em equilíbrio.
...
\section{Implementação}
    ...
    Podemos descrever os tipos implementados em uma forma mais abstrata como:
    \vskip-.9em
    \noindent%
    ...
    Podemos ver que um móbile $M$ funciona como uma árvore binária especializada, onde cada submóbile é um nó interno (com exatamente dois filhos) e cada pesos simples é uma folha. Então, a entrada foi lida em uma árvore de objetos, sendo que os pesos simples foram tratados apenas com um inteiro, o seu peso fixo $P_O$.
...
\section{Algoritmo}
    ...
    Então, podemos fortalecer a hipótese com o cálculo da função peso. Além disso, podemos usar a definição recursiva de equilíbrio, expandindo a hipótese para objetos em geral.

    \begin{hypothesisf}
        Dado um objeto $O$ com $n$ massas simples, podemos calcular seu peso $P(O)$ e vericar se ele está em equilíbrio.
    \end{hypothesisf}
    ...
    \begin{proof}[\textbf{Demonstração}] \label[proof]{proof:alg}
        Suponha um inteiro $n \geq 1$ tal que para todo objeto $O$ composto de $1 \leq k < n$ massas podemos descobrir seu peso total $P(O)$ e se ele está em equilíbrio. Suponha ainda um objeto $O$ com $n$ massas.
    ...
    \begin{description}[topsep=.4em,itemsep=-.4em,partopsep=0ex,parsep=1ex]
        ...
        \item[\textbf{Combinação:}] O peso total do móbile é a soma dos pesos dos objetos e ele estará em equilíbrio dependendo dos pesos e dos estados de equilíbrio dos objetos.
    \end{description}\vskip.4em

\section{Complexidade}

    Podemos ver pelo algoritmo que a etapa de divisão, em uma estrutura de árvore, segue trivialmente, com tempo constante $t_1$. A etapa de combinação também é uma constante $t_2$, fazendo apenas multiplicações, adições e operações lógicas. A única etapa com tempo dependente da entrada, neste algoritmo, é a conquista, implementada recursivamente.
    ...
    \begin{proof}[\textbf{Demonstração}]
        Considere as constantes $c = t_1 + t_2$ e $b = - t_2$ e suponha que $T(k) = c k + b$ para todo $1 \leq k < n$.

        \begin{ncasos}
            \item[Caso base:] $n = 1$. Logo, $T(1) = c + b = t_1 + t_2 - t_2 = t_1$, como esperado.

            \item[Passo indutivo:] Suponha que $n \geq 2$. Logo, o objeto deve ser um móbile, com filhos $E$ e $D$, com $n_e$ e $n_d$ massas simples. Como o móbile não têm massa própria, então $n_e + n_d = n$. Além disso, $1 \leq n_e < n$ e $1 \leq n_d < n$, então podemos aplicar a hipótese indutiva:
            \begin{align*}
                T(n) &= T(n_e) + T(n_d) + t_2 \\
                    &= c n_e + b + c n_d + b + t_2 = c (n_e + n_d) + b + (b + t_2) \\
                    &= c n + b
            \end{align*}
        \end{ncasos}
        \vskip-1em
    ...
    A partir da demonstração acima, podemos concluir que $T(n) \in \Theta(c n + b) = \Theta(n)$.
